...
data "azurerm_subnet" "subnet" {
  for_each             = { for inst in local.get_data : inst.unique_id => inst }
  name                 = var.subnetname
  virtual_network_name = (lookup(each.value, "vnet_name", null) != null) ? each.value.vnet_name : join("-", [local.naming.bu, local.naming.environment, local.env_location.locations_abbreviation, "vnet", local.naming.nn])
  resource_group_name  = (lookup(each.value, "vnet_resource_group", null) != null) ? each.value.vnet_resource_group : join("-", [local.naming.bu, local.naming.environment, local.env_location.locations_abbreviation, "spokenetwork-rg"])
}
